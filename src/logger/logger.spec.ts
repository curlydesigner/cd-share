...
describe('logger', () => {
    ...
    describe('log', () => {
        ...
        it('should log error when logged object has circular reference', () => {
            const obj = {};
            obj['a'] = { b: obj };
            logger.log('app name', 'app version', 'test', Logger.Level.DEBUG, [obj]);
            sinon.assert.calledWith(logStub, `[${nowForatted} app name.test]`, 'TypeError: Converting circular structure to JSON');
        })
    });
});
